...
def migrate_historic_klines_sqlite(symbol, start, end, interval):
    ...
    list_of_time_prices = []
    list_of_close_time = []
    list_of_quote_asset_volume = []
    list_number_trades = []
    list_taker_buy_base_AV = []
    list_taker_buy_quote_AV = []
    ...
    for kline in klines:
        ...
        QAV = float(kline[7])
        Number_trades = float(kline[8])
        Taker_buybaseAV = float(kline[9])
        Taker_buyquoteAV = float(kline[10])
        ...
        list_of_time_prices.append(Open_time)
        list_of_close_time.append(Close_time)
        list_of_quote_asset_volume.append(QAV)
        list_number_trades.append(Number_trades)
        list_taker_buy_base_AV.append(Taker_buybaseAV)
        list_taker_buy_quote_AV.append(Taker_buyquoteAV)
    ...
    try:
        ...
        column_values_open = pd.Series(list_of_volume_prices)
        column_values_volume = pd.Series(list_of_volume_prices)
        column_values_closetime = pd.Series(list_of_close_time)
        column_values_QAV = pd.Series(list_of_quote_asset_volume)
        column_values_number_trades = pd.Series(list_number_trades)
        column_values_TBAV = pd.Series(list_taker_buy_base_AV)
        column_values_TBQV = pd.Series(list_taker_buy_quote_AV)


        df = pd.DataFrame(dict(OpenTime = column_values_time, Open = column_values_open, Close = column_values_close, High = column_values_high, Low = column_values_low, Volume = column_values_volume, CloseTime = column_values_closetime , QAV = column_values_QAV, NumberTrades = column_values_number_trades, TBAV = column_values_TBAV, TQAV = column_values_TBQV), index = None)

        df.to_sql("ETHBTC_30MIN_historical_market_data", con , if_exists = 'replace', index = False,)
